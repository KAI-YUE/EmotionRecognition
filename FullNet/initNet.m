...
OutputVADLayers = [
    ...
    fullyConnectedLayer(3,'Name','OutputVAD');
    reluLayer('Name','VADrelu');
];

OutputCategoriesLayers = [
    fullyConnectedLayer(26,'Name','CategoriesFc');
    sigmoidLayer('OutputCategories');    
    depthConcatenationLayer(2,'Name','Output');
    OutputRegressionLayer('RegressionOutput',params);
];
    
FullNet = addLayers(FullNet,OutputVADLayers);
% Connect the separate branches to the joinTable
FullNet = connectLayers(FullNet,'Bpool','joinTable/in1');
FullNet = connectLayers(FullNet,'Ipool','joinTable/in2');

% assemble the output part
FullNet = addLayers(FullNet,OutputCategoriesLayers);
FullNet = connectLayers(FullNet,'Dropout','CategoriesFc');
FullNet = connectLayers(FullNet,'VADrelu','Output/in2');
...
BoLayers = [
    ...
    averagePooling2dLayer([3 3],'Stride',[28 28],'Name','Bpool');
